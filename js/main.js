...
function createAttPanel(attData, attLabels) {
    ...
    attWidth = window.innerWidth * 0.18,//width of attSvg
    attWidth = attWidth - attMargin.left - attMargin.right, //width with margins for padding
    pcpWidth = attHeight, pcpHeight = attWidth,
    attSpacing = attHeight / (attLabels.length + 1), //vertical spacing for each attribute
...
function createAddButton(attSvg, attributes) {
    ...
    var width = attWidth, height= attMargin.bottom - 5,       // rect dimensions
        fontSize = 1.85*height/3,    // font fills rect
        x0 = - 10, y0 = attHeight - attMargin.bottom * 1.5,
        x0Text = x0 + width/2, y0Text = y0 + 0.66*height,
        text = "Change Attributes";

    var button = attSvg.append("g")
        .attr("class", "button")
        .attr("x", x0)
        .attr("y", y0)

    //create background of button; this will be styled
    var buttonBackground = button.append("rect")  // button background
        ...
        .attr("ry", height/10);
    //add text to button
    var buttonText = button.append("text")  // button text
        .attr("class", "button")
        .attr("id","buttonText")
        .attr("x", x0Text)
        .attr("y", y0Text)
    ...
    var buttonClick = button.append("rect")
        .attr("class", "button")
        .attr("id","buttonClick")
        .attr("width", width + "px")
        .attr("height", height + "px")
        ...
        .attr("y", y0)
        .attr("ry", height/2)
        .on("click", changeAttributes);

//how to 
//     function changeAttributes(){
//         // //do we want to display all atributes with selected ones already checked? or just ones that can still be added
//         // var labelAttribute = "";
//         // //label content
//         // for (i=0; i<attributes.length; i++) {
//         //     labelAttribute += "<p>" + attributes[i] +
//         //     "</p>";
//         // }
//         // //create info label div
//         // var attSelect = d3.select("body")
//         //   .append("svg")
//         //   .attr("class", "attSelect")
//         //   .html(labelAttribute);
...
//         attWidth = attWidth - attMargin.left - attMargin.right, //width with margins for padding
//         pcpWidth = attHeight, pcpHeight = attWidth,
//         attSpacing = attHeight / (attributes.length + 1), //vertical spacing for each attribute
//         rectWidth = 4, rectHeight1 = 6, rectHeight2 = 11,
//         rectHeight3 = 16, rectSpacing = 3;
//
//         //empty array to hold length of each label
...
//         //for loop to push all label lengths into array
//         for (i=0; i<attributes.length; i++) {
//             var attLength = attributes[i].length;
//             labelLength.push(attLength)
//         }
//         //identify which label is the longest so we can use that as the width in the transform for creating text elements
//         var labelWidth = Math.max.apply(Math, labelLength);
//
//         //create svg for attpanel
//         var changeSvg = d3.select("body").append("svg")
...
//
//         //sets att title
//         var changeTitle = changeSvg.append("text")
//             .attr("class", "changeTitle")
//             .attr("x", attWidth / 5)
//             .attr("y", attMargin.top)
//             .text("Attributes")
//
//         //creates a group for each rectangle and offsets each by same amount
//         var variables = changeSvg.selectAll('.variables')
//             .data(attributes)
//             .enter()
//           .append("g")
//             .attr("class", "variables")
...
//                 var height = labelWidth + attSpacing/1.5;
//                 // console.log(height);
//                 var offset =  height * attributes.length / 1.05;
//                 // console.log(offset);
//                 var horz = -labelWidth - 5; //x value for g translate
//                 var vert = i * height - offset; //y value for g translate
//                 console.log(horz);
